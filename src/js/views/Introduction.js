...
App.Views.Introduction = Backbone.View.extend({
    ...
    nextQuestion: function(e) {
        ...
        var self = this,
            currentFieldset = $(e.currentTarget).parent(),
            nextFieldset = currentFieldset.next(),
            currentInput = currentFieldset.find('textarea');
    ...
    updateProgress: function(increase) {
        var self = this,
            incrementValue = self.$el.find('.progress-bar').width()/4,
            progressBar = self.$el.find('.progress-bar span'),
            progressNum = self.$el.find('.progress-num i'),
            progress = parseInt(progressNum.html(), 10);
    ...
    countCharacters: function(e) {
        var self = this,
            counter = $(e.currentTarget).siblings('.text-count'),
            currentCount = $(e.currentTarget).val().length,
            counterValue;
    ...
    submitAnswers: function(e) {
        ...
        var self = this,
            answers = self.$el.find('textarea'),
            currentInput = $(e.currentTarget).parentsUntil('.active').parent().find('textarea');
