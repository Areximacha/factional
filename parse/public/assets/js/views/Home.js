...
App.Views.Home = Backbone.View.extend({
    ...
    initialize: function() {
        ...
        $.ajax("assets/js/templates/home.tpl").success(function(html) {
            self.$el.empty();
            $(".loading").show();
            var isSearch = false, isFiltered = false;
            if ($("#search input[name='search']").length > 0 && $("#search input[name='search']").val() !== "") {
                isSearch = true;
            }
    ...
    addNew: function(profile) {
        var self = this, profileName = profile.get("name"), currentFirstChar = profileName.slice(0, 1), profileContainer = self.$el.find(".profiles");
    ...
    render: function(office) {
        ...
        if (!office || office == "all") {
            ...
            var profiles = App.Collections.Profiles.where({
                office: office
            });
        }
        $(".loading").hide();
